...
const PROJECTS: Project[] = [
  {
    ...
    description:
      "Modern e-commerce landing com foco em conversão, tipografia limpa e componentes reusáveis.",
...
export default function Projects() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight" && open && active) {
        setImgIdx((i) => ((i + 1) % active.images.length));
      }
      if (e.key === "ArrowLeft" && open && active) {
  ...
  return (
    <section id="projects" className="relative overflow-hidden pt-14 pb-14">
      ...
      <div className="relative z-10 container-hero">
        ...
        <div
          ref={trackRef}
          className="no-scrollbar mt-4 flex gap-6 overflow-x-auto snap-x snap-mandatory scroll-px-4 pb-2"
        >
          {/* mascara nas bordas */}
          <div className="pointer-events-none sticky left-0 w-6 shrink-0 bg-gradient-to-r from-[var(--bg)] to-transparent" />
          {PROJECTS.map((p) => (
            <article
              key={p.id}
              data-card
          ...
          <div className="pointer-events-none sticky right-0 w-6 shrink-0 bg-gradient-to-l from-[var(--bg)] to-transparent" />
        </div>
      </div>
